...
inline Window::Window(std::string windowName, Axes &axes, int rows, int cols)
    : _mouseAdapter(axes), _windowName(windowName) {
...
inline Window::Window(Window &&a)
    : _mouseAdapter(std::move(a._mouseAdapter)),
      _windowName(std::move(a._windowName)), _mat(std::move(a._mat)) {
  setMouseCallback();
}
...
inline Axes &Window::getAxes() { return _mouseAdapter.getAxes(); }
...
inline void Window::setMouseEventHandler(MouseEventHandler mouseEventHandler) {
...
inline int Window::waitKey(int delay) { return waitKey({this}, delay); }

inline int Window::waitKey(const std::vector<Window *> &windows, int delay) {
  const auto endTickCount =
      cv::getTickCount() + (int64)(delay * 0.001f * cv::getTickFrequency());
  ...
  while (true) {
    ...
    // all windows closed?
    if (std::all_of(windows.begin(), windows.end(),
                    [](Window *window) { return !window->valid(); })) {
      return -1;
    }
...
inline void Window::setMouseCallback() {
  cv::setMouseCallback(
      _windowName,
      [](int event, int x, int y, int flags, void *userdata) {
        Window &window = *static_cast<Window *>(userdata);
        window.updateSize();
        MouseEvent mouseEvent(window._mouseAdapter.getAxes(),
                              window._mat.size(), event, x, y, flags);
